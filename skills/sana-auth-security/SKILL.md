...
- Ongoing: EVERY new endpoint must be JWT-protected unless it is login
...
POST /login
  services.Login:
    ...
    4. Generate refresh token (utils.GenerateRefreshJWT, 120h, JWT_REFRESH_SECRET)
    5. Persist Session{UserEmail, RefreshToken} in snSessions
...
POST /refreshToken
  services.RefreshToken:
    ...
    3. Load user, mint NEW access + refresh tokens (ROTATION)
    4. UpdateOne on the session (old token -> new token, is_blocked=false)
  -> returns { token, refresh_token }
```
...
- **Refresh must rotate.** `RefreshToken` replaces the stored token and rejects
  blocked sessions (`MatchedCount == 0` -> error). Never echo the received token back.
...
- **JWT middleware** (`middlewares/auth.middleware.go`):
  ...
  - Public routes: only `POST /login`. Everything else gazes at `JWTProtected()`.
