...
- `apps/web` — Vite 6 + React 18 (JS/TSX in plan) + antd 5 + styled-components + pnpm
...
pnpm -C apps/web dev      # FE dev server (Vite, port 3000, proxies /api to :9000)
pnpm -C apps/web test     # FE unit tests (Vitest)
pnpm -C apps/web build    # FE production build (Vite → dist, generates PWA)
...
- **FE dev**: `vite.config.js` proxies `/api` to `http://localhost:9000`.
  `VITE_API_URL` overrides the API base for the built bundle (Dockerfile ARG,
  also `.env` if present, gitignored). Production build is served by
  `apps/web/server.js` (express, serves `dist/`). PWA manifest + SW are
  generated by `vite-plugin-pwa` at build time.
